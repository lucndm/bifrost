...
# UI build chạy tuần tự ở cuối (không dùng `npm run build` vì copy-build của nó
# ghi đè transports/bifrost-http/ui; cũng không chạy nền vì process nền giữ log
# pipe của runner khiến job treo dù step đã xong).
echo "== UI build"
