...
const (
	...
	BifrostContextKeyAdaptivePinnedAPIKeyName            BifrostContextKey = "bifrost-adaptive-pinned-api-key-name"    // string (provider key NAME pinned by the adaptive load balancer for this attempt; core normalizes it into BifrostContextKeyAPIKeyName after the pre-request phase unless the caller or a routing rule pinned a key (set by bifrost adaptive plugin - DO NOT SET THIS MANUALLY))
